...
class User extends Authenticatable implements MustVerifyEmail
{
    ...
    public function invoiceConsolidatedNumberFormat($number)
    {
    ...
    public function billConsolidatedNumberFormat($number)
    {
    ...
    public function countConsolidatedCustomers()
    {
        // return Customer::where('created_by', '=', $this->creatorId())->count();
    ...
    public function countConsolidatedVenders()
    {
        // return Vender::where('created_by', '=', $this->creatorId())->count();
        return Vender::count();
    }
    ...
    public function countConsolidatedInvoices()
    {
        // return Invoice::where('created_by', '=', $this->creatorId())->count();
    ...
    public function countConsolidatedBills()
    {
        // return Bill::where('created_by', '=', $this->creatorId())->count();
        return Bill::count();
    }
    ...
    public static function getConsolidatedBillProductsData($date = '', $month = '')
    {
        if ($month != '' && $date != '') {
            $BillProducts = \DB::table('bill_products')
                ->select('bill_products.bill_id as bill',
    ...
    public function getConsolidatedIncExpBarChartData()
    {
        $month[] = __('January');
        $month[] = __('February');
        $month[] = __('March');
        ...
        $user_id = \Auth::user()->creatorId();
        for ($i = 1; $i <= 12; $i++) {
            $monthlyIncome = Revenue::selectRaw('sum(amount * exchange_rate) amount')->whereRaw('year(`date`) = ?', array(date('Y')))->whereRaw('month(`date`) = ?', $i)->first();
            $invoiceTotal = self::getInvoiceProductsData((date('Y')), $i);
    ...
    public function getConsolidatedIncExpLineChartDate()
    {
        $usr = \Auth::user();
        $m = date("m");
        $de = date("d");
        ...
        for ($i = 0; $i < count($arrDate); $i++) {
            // $dayIncome = Revenue::selectRaw('sum(amount * exchange_rate) amount')->where('created_by', \Auth::user()->creatorId())->whereRaw('date = ?', $arrDate[$i])->first();
            $dayIncome = Revenue::selectRaw('sum(amount * exchange_rate) amount')->whereRaw('date = ?', $arrDate[$i])->first();
            ...
            $billTotal = self::getConsolidatedBillProductsData($arrDate[$i]);
    ...
    public function invoicesData($start, $current)
    {
        $InvoiceProducts = Invoice::select('invoices.invoice_id as invoice')
            ...
            // ->where('invoices.created_by', \Auth::user()->creatorId())
            ->when(auth()->user()->type != "super admin", function ($query) {
                $query->where('invoices.created_by', \Auth::user()->creatorId());
            })
            ->groupBy('invoice')
            ->get()
            ->keyBy('invoice')
    ...
    public function billsData($start, $current)
    {
        $billProducts = Bill::select('bills.bill_id as bill')
            ...
            // ->where('bills.created_by', \Auth::user()->creatorId())
             ->when(auth()->user()->type != "super admin", function ($query) {
                $query->where('bills.created_by', \Auth::user()->creatorId());
            })
